setwd(dir = 'D:/OneDrive - UGent/Doctoraat Gent/Invasion/1_bachelors')
#setwd(dir = 'D:/fremorti/OneDrive - UGent/Doctoraat Gent/Invasion')
library(car)
library(tidyverse)
library(ggplot2)
library(brms)
...
ggplot(data, aes(x = day, y = edge, color = treatment))+
  geom_point()+
  geom_smooth(method = 'lm')
#spread per mesocosm
ggplot(data, aes(x = day, y = edge, color = treatment, group = metapop))+
  geom_point()+
  geom_smooth(method = 'lm')
#spread per treatment
ggplot(data, aes(x = day, y = total, color = treatment))+
  geom_point()+
  geom_smooth(method = 'lm')
#spread per mesocosm
ggplot(data, aes(x = day, y = total, color = treatment, group = metapop))+
  geom_point()+
  geom_smooth(method = 'lm')
...
#sd of treatment
data.b <- cbind(data$treatment == '8', data$treatment == '16')
sd.b <- apply(mcmc[, 2:3]%*%t(data.b), 1, sd)
bs <- cbind(mcmc[,2:3], rep(0, 6000))

#sd of treatment:day
data.bdayp <- t(data.b*data$day)
sd.bday <- apply(mcmc[, 5:6] %*% data.bdayp, 1, sd)

#sd of metapop
data.meta <- sapply(data$metapop, function(x) levels(data$metapop) == x)
sd.metapop <- apply(mcmc[, 11:25] %*% data.meta, 1, sd)

#sd of metapop:day
data.metadayp <- data.meta*data$day
sd.metaday <- apply(mcmc[, 26:40] %*% data.metadayp, 1, sd)

#sd of residuals
sd.resid <- apply(residuals(fedge, summary = FALSE), 1, sd)

sd.all <- cbind(sd.b, sd.day, sd.bday, sd.metapop, sd.metaday, sd.resid)%>%as_tibble()%>%rename('con' = sd.b, 'day' = 'sd.day', 'c:d' = 'sd.bday', 'mesoc' = 'sd.metapop', 'm:d' = 'sd.metaday', 'resid' = 'sd.resid')
summary(sd.all)

#plot all sources of variation in the model (fig 3)
sd.all_ <- sd.all%>%pivot_longer(cols = colnames(.))
sd.all_$name <- factor(sd.all_$name, levels = c('day', 'con', 'c:d', 'mesoc', 'm:d', 'resid'))
(EvHistPlot <- ggplot(sd.all_, aes(x = name, y = value))+
  ...
  ggtitle('sources of variation in population edge', subtitle = 'connectedness treatment')+
  theme_minimal()+
  theme(legend.position = "none", text = element_text(size = 15)))

###version with both variable effects
#model spread as a function of the evolutionary history (metapop)222222222222222222222222222222222222222
ffedge <- brmsformula(edge  ~ 0 + Intercept + day + (1 + day|treatment) + (1 + day|metapop))
fedge <- brm(data = data, family = 'gaussian',
             formula = ffedge,
             prior = c(prior(normal(0,4), class = b),
                       prior(cauchy(0,2), class = sd),
...
#calculate sd of parameters2222222222222222222222222222
mcmc <- as.matrix(fedge)

#sd of day
sd.d <- sd(data$day)
sd.day <- abs(mcmc[,2])*sd.d

#sd of treatment
data.b <- sapply(data$treatment, function(x) levels(data$treatment) == x)
sd.b <- apply(mcmc[, 40:42] %*% data.b, 1, sd)


#sd of treatment:day
data.bdayp <- data.b*data$day
sd.bday <- apply(mcmc[, 43:45] %*% data.bdayp, 1, sd)

#sd of metapop
data.meta <- sapply(data$metapop, function(x) levels(data$metapop) == x)
sd.metapop <- apply(mcmc[, 10:24] %*% data.meta, 1, sd)

#sd of metapop:day
data.metadayp <- data.meta*data$day
sd.metaday <- apply(mcmc[, 25:39] %*% data.metadayp, 1, sd)
...
sd.all_ <- sd.all%>%pivot_longer(cols = colnames(.))
sd.all_$name <- factor(sd.all_$name, levels = c('day', 'con', 'con:day', 'mesoc', 'mesoc:day', 'resid'))
ggplot(sd.all_, aes(x = name, y = value))+
  ...
  xlab('level of the model')+
  ylab('standard deviation of parameters')+
  ggtitle('sources of variation in population edge', 'connectedness treatment')+
  theme_minimal()+
  theme(legend.position = "none", text = element_text(size = 15))
...
ggplot(sd.all[c(9,11,12)]%>%select('b:day_metapop:day' = 'b:day_metapop:day', 'b:day_resid' = bday_, 'metapop:day_resid' = metaday_)%>%pivot_longer(cols = colnames(.)), aes(name, value))+
  ...
  scale_x_discrete(labels = c('c:d / m:d', 'c:d / resid', 'm:d / resid'))+
...
#calculate group estimates from posterior
...
#plot highest likelihood slopes per treatment (and 91% spread)
...
LHsd.all <- cbind(LHsd.day, LHsd.repr, LHsd.reprday, sd.metapop, sd.metaday, sd.resid)%>%as_tibble()%>%rename('day' = 'LHsd.day', 'repr' = LHsd.repr, 'r:d' = LHsd.reprday, 'mesoc' = 'sd.metapop', 'm:d' = 'sd.metaday', 'resid' = 'sd.resid')
summary(LHsd.all)

#plot sources of variation (fig 6)
LHsd.all_ <- LHsd.all%>%pivot_longer(cols = colnames(.))
LHsd.all_$name <- factor(LHsd.all_$name, levels = c('day', 'repr', 'r:d', 'mesoc', 'm:d', 'resid'))
(LHPlot<-ggplot(LHsd.all_, aes(x = name, y = value))+
  geom_violin(draw_quantiles = c(0.09, 0.5, 0.91), scale = 'width', aes(fill = name))+
  scale_fill_brewer( palette = "Set2")+
  ylim(0, 4)+
  xlab('level of the model')+
  ylab('standard deviation of parameters')+
  ggtitle('', subtitle = 'reproductive success')+
  theme_minimal()+
  theme(legend.position = "none", text = element_text(size = 15)))
...
dsd.all <- cbind(dsd.day, dsd.disp, dsd.dispday, sd.metapop, sd.metaday, sd.resid)%>%as_tibble()%>%rename('day' = 'dsd.day', 'disp' = dsd.disp, 'di:d' = dsd.dispday, 'mesoc' = 'sd.metapop', 'm:d' = 'sd.metaday', 'resid' = 'sd.resid')
summary(dsd.all)

#plot sources of variation in the model (fig 8)
dsd.all_ <- dsd.all%>%pivot_longer(cols = colnames(.))
dsd.all_$name <- factor(dsd.all_$name, levels = c('day', 'disp', 'di:d', 'mesoc', 'm:d', 'resid'))
(DispPlot <- ggplot(dsd.all_, aes(x = name, y = value))+
  geom_violin(draw_quantiles = c(0.09, 0.5, 0.91), scale = 'width', aes(fill = name))+
  scale_fill_brewer( palette = "Set2")+
  ylim(0, 4)+
  xlab('level of the model')+
  ylab('standard deviation of parameters')+
  ggtitle('', subtitle = 'dispersal propensity')+
  theme_minimal()+
  theme(legend.position = "none", text = element_text(size = 15)))
...
(d1_ <- ggplot(postd_, aes(x = disp, y = Estimate))+
  ...
  geom_ribbon(aes(ymin = Q9, ymax = Q91), fill = "black", alpha = 0.1) +
  ylab('estimated slope')+
  xlab('dispersal propensity')+
  ggtitle('dispersal effect on spread rate'))+
  theme_minimal()+
  theme(legend.position = "none", text = element_text(size = 15))

grid.arrange(grobs = list(LH1_, d1_), ncol = 2)


ggplot(data3, aes(x = repr, y = disp))+
  geom_point()
ggplot(data3[data3$day == 27,], aes(x = repr, y = total))+
  geom_point()
ggplot(data3[data3$day == 27,], aes(x = total, y = disp))+
  geom_point()

grid.arrange(grobs = list(EvHistPlot, LHPlot, DispPlot), ncol = 3)
